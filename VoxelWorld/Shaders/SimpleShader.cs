...
void main(void)
{
    ...
    uint x = (uint(gl_VertexID) - baseVertexIndex) % uint(size.x);
    uint y = ((uint(gl_VertexID) - baseVertexIndex) / uint(size.x)) % uint(size.y);
    uint z = ((uint(gl_VertexID) - baseVertexIndex) / (uint(size.x) * uint(size.y)));
